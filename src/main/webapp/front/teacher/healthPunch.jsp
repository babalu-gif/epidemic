...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#exportNotClockStudentsBtn").click(function () {
            // 发送同步请求
            window.location.href="exportNotClockStudents.do?ci=${sessionScope.sessionTeacher.class_id}";
        });

        // 为“导出未提交行程码”按钮添加单击事件
        $("#exportNotTourStudentsBtn").click(function () {
            // 发送同步请求
            window.location.href="exportNotTourStudents.do?ci=${sessionScope.sessionTeacher.class_id}";
        });

        // 为“导出未提交健康码”按钮添加单击事件
        $("#exportNotHealthStudentsBtn").click(function () {
            // 发送同步请求
            window.location.href="exportNotHealthStudents.do?ci=${sessionScope.sessionTeacher.class_id}";
        });

        // 为向未打卡学生“发送邮件”按钮添加单击事件
        $("#sendEmailToNotClockBtn").click(function () {
            $.ajax({
                url:"sendEmailToNotClock.do?ci=${sessionScope.sessionTeacher.class_id}",
                type:"post",
                dataType:"json",
    ...
    function refresh(page, pageSize) {
        ...
        $.post("queryPunchByConditionAndClassId.do", {
            "page": page,
            "pageSize": pageSize,
